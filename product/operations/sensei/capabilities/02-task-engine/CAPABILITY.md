...
> **Contact Compliance gate**: Before creating any contact task (Call / Visit), Task Engine queries the BOS collection note log. If the customer's daily contact limit is already reached, the task is suppressed and not created. Admin and manual tasks are not gated.
...
Before generating a contact task (Call / Visit), Task Engine queries the BOS collection note log:

| Result | Behavior |
|--------|----------|
| `contact_count < daily_limit` | Task is created normally |
| `contact_count >= daily_limit` | Task is **not created**; contract flagged in supervisor exception panel as "Contact limit reached" |

> **Dependency**: BOS collection note log — Sensei does not own contact count data. DaVinci owns the log; Sensei reads it.

### Contact Window Rule

| Event | Behavior |
|-------|----------|
| `ContactWindowClosed` (from DaVinci) | Do not create contact tasks outside business hours; CO sees "Contact window closed" indicator |

### ContactRecorded Feedback Contract

Published to DaVinci after every Call or Visit task closure:
...
| Contact pre-check latency | BOS collection note log query must complete within 500ms; on timeout, fail-safe default is task suppression (conservative — never over-contact) |
| No contact data ownership | Sensei must not maintain its own contact frequency count — always read from BOS collection note log |
| ContactRecorded always published | On every Call / Visit task closure, ContactRecorded must be published to DaVinci (not optional) |
