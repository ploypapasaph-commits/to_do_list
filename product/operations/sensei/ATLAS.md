...
| D9 | **Trust-but-verify via BOS collection note log** | COs could fabricate outcomes (record "called" without calling). Direct blocking would slow down legitimate work. | Sensei cross-references recorded Call outcomes against BOS collection notes after the fact. Flags discrepancies in supervisor exception panel. Does not block COs in real-time — maintains throughput while enabling accountability. |
